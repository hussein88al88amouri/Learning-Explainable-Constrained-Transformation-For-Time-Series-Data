...
def cop_kmeans(dataset, k, ml=[], cl=[],
               metric='l2_distance', type_='dependent',
               initialization='kmpp', max_iter=100, tol=1e-4):

    ml, cl = transitive_closure(ml, cl, len(dataset))
    ml_info = get_ml_info(ml,
                          dataset,
                          metric=metric)
    tol = tolerance(tol, dataset)
    centers = initialize_centers(dataset,
                                 k,
                                 initialization,
                                 metric=metric,
                                 type_=type_)
    ...
    for _ in range(max_iter):
        ...
        for i, d in enumerate(dataset):
            indices, _ = closest_clusters(centers,
                                          dataset[[i]],
                                          metric=metric,
                                          type_=type_)
            counter = 0
            if clusters_[i] == -1:
                ...
                if not found_cluster:
                    return None, None
        clusters_, centers_ = compute_centers(clusters_,
                                              dataset,
                                              k,
                                              ml_info,
                                              metric=metric,
...
def dtw_distance(point1, point2, type_='dependent'):
    ...
    point1 = np.array(point1) if not isinstance(point1, np.ndarray) else point1
    point2 = np.array(point2) if not isinstance(point2, np.ndarray) else point2
    if type_ == 'dependent':
...
def l2_distance(point1, point2):
    return cdist(point1[0, :, :].reshape((1, -1)),
                 point2[0, :, :].reshape((1, -1)),
                 'euclidean')[0][0] ** 2
...
def tolerance(tol, dataset):
    ...
    averages = [sum(dataset[i][d] for i in range(n))/float(n)
                for d in range(tsLenght)]
    variances = [sum((dataset[i][d] - averages[d]) ** 2
...
def closest_clusters(centers, datapoint,
                     metric='l2_distance', type_='dependent'):
    """AI is creating summary for closest_clusters

    Args:
        centers ([type]): [description]
        datapoint ([type]): [description]
        metric (str, optional): [description]. Defaults to 'l2_distance'.
        type_ (str, optional): [description]. Defaults to 'dependent'.

    Returns:
        [type]: [description]
    """
    if metric == 'l2_distance':
        distances = [l2_distance(centers[[center]],
                                 datapoint)
                     for center in range(len(centers))]
    elif metric == 'dtw_distance':
        distances = [dtw_distance(centers[[center]],
                                  datapoint,
                                  type_=type_)
                     for center in range(len(centers))]
...
def initialize_centers(dataset, k, method,
                       metric='l2_distance', type_='dependent'):
    """AI is creating summary for initialize_centers
        ...
        for _ in range(k):
            ...
            if metric == 'l2_distance':
                ...
                for index, point in enumerate(dataset):
                    point = point.reshape(-1, tsLength, dim)
                    cids, distances = closest_clusters(centers_arr,
                                                       point,
                                                       metric=metric,
                                                       type_=type_)
...
def violate_constraints(data_index, cluster_index, clusters, ml, cl):
    """AI is creating summary for violate_constraints

    Args:
        data_index ([type]): [description]
        cluster_index ([type]): [description]
...
def compute_centers(clusters, dataset, k, ml_info,
                    metric='l2_distance', type_='dependent'):
    """AI is creating summary for compute_centers

    Args:
        clusters ([type]): [description]
        dataset ([type]): [description]
        k ([type]): [description]
        ml_info ([type]): [description]
        metric (str, optional): [description]. Defaults to 'l2_distance'.
        type_ (str, optional): [description]. Defaults to 'dependent'.
    ...
    if metric == 'l2_distance':
        ...
        for cluster_id in id_map.values():
            ...
            centers[cluster_id] = dtwDBA(dataset[group].reshape(-1,
                                                                tsLength,
                                                                dim))
    ...
    if k_new < k:
        ...
        if metric == 'l2_distance':
            ...
            current_scores = [sum(dtw_distance(centers[[clusters[i]]],
                                               dataset[[i]], type_=type_)
                                  for i in group)
                              for group in ml_groups]
            group_ids = sorted(range(len(ml_groups)),
                               key=lambda x: current_scores[x] - ml_scores[x],
...
def get_ml_info(ml, dataset, metric='l2_distance', type_='dependent'):
    ...
    if metric == 'l2_distance':
        ...
        nbcentroids = len(groups)
        tsLength, dim = dataset.shape[1], dataset.shape[2]
...
def transitive_closure(ml, cl, n):
    """AI is creating summary for transitive_closure
    ...
    for i in ml_graph:
        for j in ml_graph[i]:
            if j != i and j in cl_graph[i]:
                raise Exception('inconsistent constraints between %d and %d'
                                % (i, j))
